...
SInt32 ReflectorStream::AddOutput(ReflectorOutput* inOutput, SInt32 putInThisBucket)
{
    ...
    for(UInt32 y = 0; y < sBucketSize; y++)
    {
        ...
        {
			qtss_printf("--------------- ReflectorStream::AddOutput  sfOutputArray[%d][%d] == NULL,  sBucketSize=%d-- \n", putInThisBucket, y, sBucketSize);
...
QTSS_Error ReflectorStream::BindSockets(QTSS_StandardRTSP_Params* inParams, UInt32 inReflectorSessionFlags, Bool16 filterState, UInt32 timeout)
{
    ...
    qtss_printf("--------- ReflectorStream::BindSockets,  fStreamInfo.fPort:%d, fStreamInfo.fSrcIPAddr:%d  --------- \n", 
			fStreamInfo.fPort, 
    ...
    if (fSockets == NULL){
        ...
        qtss_printf("--------- ReflectorStream::BindSockets,  fSockets == NULL  --------- \n");
        return QTSSModuleUtils::SendErrorResponse(inRequest, qtssServerInternal,
                                                    sCantBindReflectorSocketErr);

	}

    qtss_printf("--------- ReflectorStream::BindSockets,  fSockets->GetSocketA()->GetLocalAddrStr()->Ptr:%s, port:%s --------- \n", 
		fSockets->GetSocketA()->GetLocalAddrStr()->Ptr, fSockets->GetSocketA()->GetLocalPortStr()->Ptr);
    ...
    if (((ReflectorSocket*)fSockets->GetSocketA())->HasSender() && (fStreamInfo.fSrcIPAddr == 0)){
    	...
    	qtss_printf("--------- ReflectorStream::BindSockets, Error");
        return QTSSModuleUtils::SendErrorResponse(inRequest, qtssServerInternal,
                                                    sCantBindReflectorSocketErr);
    	}
...
void ReflectorSender::ReflectPackets(SInt64* ioWakeupTime, OSQueue* inFreeQueue)
{
    ...
    //qtss_printf("ReflectPackets::fStream->fNumBuckets= %d, fStream->sBucketSize:%d\n", fStream->fNumBuckets, fStream->sBucketSize);
    for (UInt32 bucketIndex = 0; bucketIndex < fStream->fNumBuckets; bucketIndex++)
    {   
        for (UInt32 bucketMemberIndex = 0; bucketMemberIndex < fStream->sBucketSize; bucketMemberIndex++)
        {    
...
/*
        if(fLastRTPNum >= thePacket->GetPacketRTPSeqNum() || (fWriteFlag &  qtssWriteFlagsIsRTCP))
        {
            qIter.Next();
            printf("---- fLastRTPNum=%ld, seqNum=%ld--  lost \n", fLastRTPNum, thePacket->GetPacketRTPSeqNum());
            continue;
        }
*/
	 if (thePacket->GetPacketRTPSeqNum() == 0){
            qIter.Next();
            continue;             
        } 
        // printf("^^^^^^^^^^^^^^^^^^^^^^^ ReflectorSender::SendPacketsToOutput, fLastRTPNum=%ld, seqNum=%ld,  WritePacket length=%ld, rtptime:%ld\n", fLastRTPNum, 
         //   thePacket->GetPacketRTPSeqNum(), 
           // thePacket->fPacketPtr.Len, thePacket->fTimeArrived  ); 
