...
:   fClientSession(inClientSession),
    ...
    fMustSynch(true),
    fPreFilter(true),
    fFirstMustIFrame(true)//lyz@xdja.com add for first I frame.
{
    if(inReflectorSession != NULL)
...
{ 
	...
	/*
    if (fFirstMustIFrame){
        if (!IsIFramePacket(inPacket)){            
            return true;
        }else{
            fFirstMustIFrame = false;//
        }
    }*/
    //lyz@xdja.com add end.
...
QTSS_Error  RTPSessionOutput::WritePacket(StrPtrLen* inPacket, void* inStreamCookie, UInt32 inFlags, SInt64 packetLatenessInMSec, SInt64* timeToSendThisPacketAgain, UInt64* packetIDPtr, SInt64* arrivalTimeMSecPtr, Bool16 firstPacket)
{
    ...
    OSMutexLocker locker(&fOutputMutex);//lyz@xdja.com
...
Bool16 RTPSessionOutput::IsIFramePacket(StrPtrLen* inPacket)//lyz@xdja.com add
{
    if (inPacket == NULL || inPacket->Len < 14){
        qtss_printf(" ***RTPSessionOutput::ERROR");
        return true;
    }
    
    UInt8* payload = (UInt8*)inPacket->Ptr;
    int start = 12;//rtp header length
    int rtptype = payload[start] & 0x1f;
    int payload_type = payload[1]&0x7f;
    qtss_printf(" ***RTPSessionOutput::IsIFramePacket fFirstMustIFrame:%d rtptype:%d, payload_type:%d, this:%08x\n", fFirstMustIFrame, rtptype, payload_type, this);
    /*
    if (payload_type != 96){
        return true;//
    }
    */
    ...
    if (rtptype== 28) {
        // FU-A
        int type = payload[start + 1] & 0x1F;

        qtss_printf(" ***RTPSessionOutput::IsIFramePacket type:%d\n", type);
        if (type == 5){
            return true;
        }
    } else if (rtptype <= 24){
        // single nal unit
        if (rtptype == 5 || rtptype == 7 || rtptype == 8){
            return true;
        }
    }
...
RTPSessionSaveAsMP4::RTPSessionSaveAsMP4(ReflectorSession* inReflectorSession, char* theFileName)
    :fReflectorSession(inReflectorSession),
    ffd(-1)
{
    ...
    if (firstPos != NULL){
        ...
        if (i > 0 && end > i + 1){
            ...
            fFileName[end - i - 1] = '\0';
            sprintf(fFilePath, "%s%s", DEFAULTPATH, fFileName);
            printf("----fFileName:%s, path:%s-------\n", fFileName, fFilePath);
        }
    }
    
    MkFileDirandFile();

    printf("-------  RTPSessionSaveAsMP4,  GetNumStreams=%d \n", inReflectorSession->GetNumStreams());
    this->InititializeBookmarks( inReflectorSession->GetNumStreams());

}


RTPSessionSaveAsMP4::~RTPSessionSaveAsMP4()
{
...
bool RTPSessionSaveAsMP4::MkFileDirandFile()
{        
    if(access(fFilePath, R_OK) != 0) // dir exists
    {
      int isCreate = mkdir(fFilePath , S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);
      if( !isCreate )    
        qtss_printf("create dir ok, %s\n", fFileName);
      else
      {
        qtss_printf("create %s failed! error code : %d\n", fFilePath, isCreate);
        return false;
      }
    }

   qtss_printf("---------- dir is ok ---------------%s\n", fFileName);    
...
ReflectorSession* RTPSessionSaveAsMP4::GetReflectorSession() 
{
    qtss_printf("################################## RTPSessionSaveAsMP4::GetReflectorSession \n");
    return fReflectorSession; 
}

void RTPSessionSaveAsMP4::Write_Header()
{
    if (1){
...
QTSS_Error  RTPSessionSaveAsMP4::WritePacket(StrPtrLen* inPacket, void* inStreamCookie, UInt32 inFlags, SInt64 packetLatenessInMSec, SInt64* timeToSendThisPacketAgain, UInt64* packetIDPtr, SInt64* arrivalTimeMSecPtr, Bool16 firstPacket)
{
    if (inPacket == NULL || inPacket->Len < 14){
         
                qtss_printf("inPacket == NULL || inPacket->Len < 14");
	  return QTSS_NoErr;
    }
   ...
   if(-1 != ffd)
   {
	...
	//check end.
        ...
        //qtss_printf("#################   rtptype  = %d \n", rtptype);
        if (lastArrivalTimeMSec >= *arrivalTimeMSecPtr){
            return QTSS_NoErr;
        }
        qtss_printf("RTPSessionSaveAsMP4::WritePacket lastArrivalTimeMSec=%d\n", lastArrivalTimeMSec);
	 ...
	 int ret = 0;
        ...
        if (rtptype== 28) { // FU-A
            ...
            //printf("################# rtptype== 28, arrivalTimeMSecPtr=%ld, len =%d\n", *arrivalTimeMSecPtr, inPacket->Len - start -2);
            if(bStartFlg) 
            {
                Write_Header();
            }
            ...
            if(ret == -1)
            {
                qtss_printf("error=%d, strerror=%s", errno, strerror(errno));
            }               
        } else if (rtptype <= 24){
            // single nal unit
            // printf("####################### rtptype rtptype =%d, arrivalTimeMSecPtr=%ld, len =%d\n", rtptype ,*arrivalTimeMSecPtr,  (inPacket->Len - start));
        ...
        adtsHeader[6] = 0xFC;
        ret = write(ffd, adtsHeader, sizeof(adtsHeader));
        
        ret = write(ffd, (void*)(payload + start + skip), (inPacket->Len - start - skip));
        if(ret == -1)
        {
            qtss_printf("error=%d, strerror=%s", errno, strerror(errno));
        }
