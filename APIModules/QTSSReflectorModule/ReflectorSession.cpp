...
ReflectorSession::~ReflectorSession()
{
    ...
    {
        qtss_printf(" ------------- ~ReflectorSession, fSteamArray == NULL \n");
    }

    if(fSourceInfo == NULL)
    {
        qtss_printf(" ------------- ~ReflectorSession, fSourceInfo == NULL \n");
    }
    if(fStreamArray != NULL) 
    delete [] fStreamArray;
    if(fSourceInfo != NULL)
...
{   
    ...
    qtss_printf("SetupReflectorSession fSourceInfo->GetNumStreams()=%d\n", fSourceInfo->GetNumStreams());
...
void    ReflectorSession::AddOutput(ReflectorOutput* inOutput, Bool16 isClient)
{
    ...
    SInt32 bucket = -1;
    SInt32 lastBucket = -1;
    qtss_printf("AddOutput fSourceInfo->GetNumStreams()=%d\n", fSourceInfo->GetNumStreams());
    while (true)
    {
        UInt32 x = 0;
