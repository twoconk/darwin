...
typedef OSHashTable<RTPSessionSaveAsMP4, SaveAsMP4TableKey> saveAsMP4HashTable;
static saveAsMP4HashTable*  saveAsMP4Map = NULL;
...
QTSS_Error  QTSSReflectorModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
    switch (inRole)
    {
        case QTSS_Register_Role:
            qtss_printf("QTSSReflectorModuleDisPathch  QTSS_Register_Role----- \n");
            return Register(&inParams->regParams);
        case QTSS_Initialize_Role:
            qtss_printf("QTSSReflectorModuleDisPathch   QTSS_Initialize_Role----- \n");
            return Initialize(&inParams->initParams);
        case QTSS_RereadPrefs_Role:
            qtss_printf("QTSSReflectorModuleDisPathch   QTSS_RereadPrefs_Role----- \n");
            return RereadPrefs();
        case QTSS_RTSPRoute_Role:
            qtss_printf("QTSSReflectorModuleDisPathch   QTSS_RTSPRoute_Role: RedirectBroadcast ----- \n");
            return RedirectBroadcast(&inParams->rtspRouteParams);
        case QTSS_RTSPPreProcessor_Role:
             qtss_printf("QTSSReflectorModuleDisPathcha:%08x  QTSS_RTSPPreProcessor_Role: ProcessRTSPRequest----- \n", 
			 	inParams->rtspRequestParams);
             return ProcessRTSPRequest(&inParams->rtspRequestParams);
        case QTSS_RTSPIncomingData_Role:
            qtss_printf("QTSSReflectorModule:ProcessRTPData----- \n");
             return ProcessRTPData(&inParams->rtspIncomingDataParams);
        case QTSS_ClientSessionClosing_Role:
            qtss_printf("QTSSReflectorModuleDisPathch:%08x    QTSS_ClientSessionClosing_Role:DestroySession----- \n",
					inParams->clientSessionClosingParams);
            return DestroySession(&inParams->clientSessionClosingParams);
        case QTSS_Shutdown_Role:
             qtss_printf("QTSSReflectorModuleDisPathch   QTSS_Shutdown_Role----- \n");
            return Shutdown();
        case QTSS_RTSPAuthorize_Role:
            qtss_printf("QTSSReflectorModuleDisPathch   QTSS_RTSPAuthorize_Role----- \n");
            return ReflectorAuthorizeRTSPRequest(&inParams->rtspRequestParams);
        case QTSS_Interval_Role:
            qtss_printf("QTSSReflectorModuleDisPathch  QTSS_Interval_Role----- \n");
            return IntervalRole();
   }
    return QTSS_NoErr;
}
...
QTSS_Error Initialize(QTSS_Initialize_Params* inParams)
{
	...
	saveAsMP4Map = NEW saveAsMP4HashTable(1024);
...
QTSS_Error ProcessRTPData(QTSS_IncomingData_Params* inParams)
{
    ...
    qtss_printf("QTSSReflectorModule:ProcessRTPData zfl TEST\n");
    ...
    if (1)
    {
        ...
        {   theStream = theSession->GetStreamByIndex(inIndex);
            ...
            {   if (packetChannel & 1)
                ...
                qtss_printf("-------RTSP packet------channel=%u,  packetDataLen=%u \n", (UInt16) packetChannel, packetDataLen);
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params* inParams)
{
    ...
    switch (*theMethod)
    {
        ...
        //case qtssRecordMethod:
          //   return DoPlay(inParams, (*(RTPSessionSaveAsMP4**)theOutput)->GetReflectorSession());
...
ReflectorSession* DoSessionSetup(QTSS_StandardRTSP_Params* inParams, QTSS_AttributeID inPathType,Bool16 isPush, Bool16 *foundSessionPtr, char** resultFilePath)
{
    ...
    if (sAllowNonSDPURLs && !isPush)
    {
		...
		//temp code lyz@xdja.com
		if (thePathPtr.Ptr != NULL){
			char *firstPos = ::strstr(thePathPtr.Ptr, ".sdp");
			if (firstPos != NULL){
				int length = strlen(thePathPtr.Ptr);
				int i = 0;
				while(i < length){
					if (thePathPtr.Ptr[i] == firstPos[0]){
        				printf("----setup path:%s, i:%d-------\n", thePathPtr.Ptr, i);
				...
				if (j >= (length -1)){
					thePathPtr.Ptr[i+4] = '\0';
				}else{
		            ::memmove(&thePathPtr.Ptr[i+4],&(thePathPtr.Ptr[j]), (length - j + 1));
				}
			}
...
QTSS_Error DoAnnounce(QTSS_StandardRTSP_Params* inParams)
{
    ...
    qtss_printf("DoAnnounce  000 theFullPathStr =%s\n", theFullPathStr);
    ...
    qtss_printf("DoAnnounce  111 pathOK =%d\n", pathOK);
    ...
    if ((theErr != QTSS_NoErr) || (theLen != sizeof(UInt32)))
    {
      ...
      qtss_printf("DoAnnounce  222 theContentLenP =%d\n", *theContentLenP);
    ...
    qtss_printf("DoAnnounce  333 theRequestBody =%s\n", theRequestBody);
    if (theErr != QTSS_NoErr)
    {
        //
    ...
    if ((theErr == QTSS_WouldBlock) || (theLen < (*theContentLenP - theBufferOffset)))
    {
        ...
        qtss_printf("DoAnnounce  99999 [Our DoAnnounce function will get called again when there is more data]\n");
        Assert(theErr == QTSS_NoErr);
        return QTSS_NoErr;
    }
...
//
    qtss_printf("DoAnnounce  444 killBroadcast =%d\n", killBroadcast);
    if (killBroadcast)
    {  
...
// ------------ Check the headers
    ...
    qtss_printf("DoAnnounce  555 editedSDPSPL =%s\n", editedSDPSPL.Ptr);
    ...
    qtss_printf("DoAnnounce  666 theFullPath =%s\n", theFullPath.Ptr);
...
// ------------ Write the SDP 
   ...
   // sortedSDP.GetMediaHeaders()->PrintStrEOL();
    ...
    if (theSDPFile != NULL)
    {  
        qtss_printf("DoAnnounce  777 sessionHeaders =%s\n\nmediaHeaders=%s", sessionHeaders, mediaHeaders);
...
{   
    ...
    {
        printf(" ----FindOrCreateSession ,  theSessionRef == NULL , inPath =%s------- \n", inPath->Ptr);
        ...
        printf("--------------------- FindOrCreateSession, theFileData.Len=%d \n\n", theFileData.Len);      
        if (theFileData.Len <= 0)
            return NULL;

        printf("--------------------- FindOrCreateSession, theFileData.Ptr=%s \n\n\n", theFileData.Ptr);    
    ...
    {
        printf("--- FindOrCreateSession, theSessionRef != NULL  come in------- \n");    
        // Check if broadcast is allowed before doing anything else
        // At this point we know it is a definitely a reflector session
        // It is either incoming automatic broadcast setup or a client setup to view broadcast
        // In either case, verify whether the broadcast is allowed, and send forbidden response
...
QTSS_Error DoSetup(QTSS_StandardRTSP_Params* inParams)
{
    ...
    if (isPush)
    {
        ...
        {   
		 ...
		 // if(theTrackID == 0) // qtssVideoPayloadType
   //      {	 
   //          //qtss_printf("--- QTSSReflectorModule.cpp:DoSetup is push setup:  111 GetNumStreams=%d \n", theSession->GetSourceInfo()->GetNumStreams());
   //          //theFullPath.Ptr[strlen(theFullPath.Ptr)- sSDPSuffix.Len] = 0;           
   //          RTPSessionSaveAsMP4* theSaveAsMP4 = NEW RTPSessionSaveAsMP4(theSession, theFullPath.Ptr);
   //          qtss_printf("--- QTSSReflectorModule.cpp:DoSetup is push setup:  111 theSaveAsMP4=%08x \n", theSaveAsMP4);           
   //          theSession->AddOutput(theSaveAsMP4, false);            
   //          saveAsMP4Map->Add(theSaveAsMP4);
   //      }
        ...
        qtss_printf("QTSSReflectorModule:DoSetup   theReceiveBroadcastStreamPort = %d, inParams->inClientSession:%08x, theSession:%08x\n" ,
			theStreamInfo->fPort, inParams->inClientSession, theSession);
    ...
    printf("QTSSReflectorModule thePayloadName=%s , thePayloadType=%d , theTrackID=%d \n", thePayloadName->Ptr, (UInt32)thePayloadType, theTrackID);
...
{
    qtss_printf("QTSSReflectorModule::DoPlay come in\n");
    QTSS_Error theErr = QTSS_NoErr;
    UInt32 flags = 0;
    UInt32 theLen = 0;
    ...
    {   if (!sDefaultBroadcastPushEnabled)
            return QTSS_RequestFailed;
        qtss_printf("QTSSReflectorModule::DoPlay inSession == NULL\n");
	    ...
	    qtss_printf("QTSSReflectorModule::DoPlay, path:%s \n", thePathPtr.Ptr);
    ...
    {  
        ...
        if ((theErr != QTSS_NoErr) || (theLen != sizeof(RTPSessionOutput*)) || (theOutput == NULL)){
            ...
            qtss_printf("DoPlay return failed.\n");
            return QTSS_RequestFailed;
        ...
        printf("QTSSReflectorModule::DoPlay  rtpInfoEnabled=%d \n", rtpInfoEnabled);
        if (rtpInfoEnabled)
        {
        ...
        {
		    printf("QTSSReflectorModule::DoPlay  QTSS_Play  rtpInfoEnabled = 0 \n");
...
QTSS_Error DestroySession(QTSS_ClientSessionClosing_Params* inParams)
{
    ...
    printf("-- DestroySession  sSessionMap= %08x, inParams->inClientSession:%08x  ----- \n", sSessionMap, inParams->inClientSession);
    OSMutexLocker locker (sSessionMap->GetMutex());
    ...
    {   
         printf("-- DestroySession  inParams= %08x theSession != %08x----- \n", inParams, theSession);
        ...
        theErr = QTSS_SetValue(inParams->inClientSession, sClientBroadcastSessionAttr, 0, &deletedSession, sizeof(deletedSession));
			 ...
			 /*lyz add for ref session.*/
             /*
    		 OSRef* theSessionRef = theSession->GetRef();
			 if (theSessionRef != NULL){
			 	//theSessionRef->DecRefCount();
            	qtss_printf("-DestroySession theSessionRef->GetRefCount()=%d \n", theSessionRef->GetRefCount() );
			 }
		...
		if(saveAsMP4Map->GetNumEntries() > 0)
        {
            SaveAsMP4TableKey theKey(theSession->GetInitTimeMS());
            RTPSessionSaveAsMP4* saveAsMP4 =  saveAsMP4Map->Map(&theKey);
            
            outputPtr = (ReflectorOutput*) saveAsMP4;
            theSession->RemoveOutput(outputPtr, false);
            saveAsMP4Map->Remove(saveAsMP4);
            delete outputPtr;
        }
...
void RemoveOutput(ReflectorOutput* inOutput, ReflectorSession* inSession, Bool16 killClients)
{
    ...
    if (inSession != NULL)
    {
        ...
        {               
            ...
            qtss_printf("QTSSReflectorModule.cpp:RemoveOutput refcount =%d , GetNumStreams=%d \n", inSession->GetRef()->GetRefCount(), inSession->GetNumStreams() );
            ...
            qtss_printf("RemoveOutput-theSessionRef->GetRefCount()=%d \n", theSessionRef->GetRefCount() );
            if (theSessionRef->GetRefCount() <= 0)
            {   
...
{   
   ...
   // return true;//lyz@xdja.com add for xianyang
    ...
    {   
        ...
        printf("return true");
           ...
           return true;//lyz@xdja.com modify for bug
	}
        else
           return true;
    }
...
{   
        ...
        //debug_printf("ReflectorAuthorizeRTSPRequest");
