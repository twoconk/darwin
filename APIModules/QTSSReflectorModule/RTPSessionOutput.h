...
class RTPSessionOutput : public ReflectorOutput
{
    ...
    private:
        ...
        Bool16                  fMustSynch;
        Bool16                  fPreFilter;
        Bool16                  fFirstMustIFrame;//lyz@xdja.com add
        OSMutex     fOutputMutex;//lyz@xdja.com add
        
        Bool16 IsIFramePacket(StrPtrLen* inPacket);//lyz@xdja.com add
...
//---------------- zfl@xdja.com add
class SaveAsMP4TableKey;

class RTPSessionSaveAsMP4 : public ReflectorOutput
{
public:

        RTPSessionSaveAsMP4(ReflectorSession* inReflectorSession, char* theFileName = NULL);
        virtual ~RTPSessionSaveAsMP4();
...
private:
         ReflectorSession*       fReflectorSession;
         char        			 fFilePath[MaxPathSize];
         char        			 fFileName[255];
         SInt32      			 ffd;  // file handle
         Bool16  bLastBlockFlg;//
         SInt64  lastArrivalTimeMSec;
        RTPSessionSaveAsMP4*    fNextHashEntry;
        
        friend class SaveAsMP4TableKey;
        friend class OSHashTable<RTPSessionSaveAsMP4, SaveAsMP4TableKey>;         
};
...
public:
    ...
    SaveAsMP4TableKey(SInt64 iTimes) : fHashValue(iTimes){}
    ~SaveAsMP4TableKey() {}
...
private:
    ...
    SaveAsMP4TableKey(RTPSessionSaveAsMP4 *elem) 
		: fHashValue(elem->fReflectorSession->GetInitTimeMS()) {}
		
    friend int operator ==(const SaveAsMP4TableKey &key1, const SaveAsMP4TableKey &key2)
    {
