...
QTSS_Error  QTSSRelayModuleDispatch(QTSS_Role inRole, QTSS_RoleParamPtr inParams)
{
    switch (inRole)
    {
        case QTSS_Register_Role:
            qtss_printf("-- 000 QTSSRelayModuleDispatch  QTSS_Register_Role  ----- \n"); 
            return Register(&inParams->regParams);
        case QTSS_Initialize_Role:
            qtss_printf("-- 111 QTSSRelayModuleDispatch  QTSS_Initialize_Role  ----- \n"); 
        ...
        case QTSS_RTSPPostProcessor_Role:
            qtss_printf("-- 222 QTSSRelayModuleDispatch  QTSS_RTSPPostProcessor_Role  ----- \n"); 
...
{
    qtss_printf("-- 000 Relay SetupAnnouncedSessions  come in  ----- \n");    
    ...
    if ((theErr != QTSS_NoErr) || (theLen != sizeof(QTSS_RTPTransportMode)))
    {
     ...
     qtss_printf("-- 1111 Relay SetupAnnouncedSessions theMethod=%d----- \n", *theMethod);           
    ...
    // setup = true;
    qtss_printf("-- TEst 00 Relay SetupAnnouncedSessions setup=%d, play=%d----- \n", setup, play);   
    // We are only interested in SETUP mode=receive or mode=record and PLAY or RECORD requests
    if (!(setup || play))
        return QTSS_NoErr;

    qtss_printf("-- TEst 0 Relay SetupAnnouncedSessions ----- \n");
    ...
    if ((theErr != QTSS_NoErr) || (theLen != sizeof(QTSS_RTSPStatusCode)))
    {
     ...
     qtss_printf("-- TEst 1 Relay SetupAnnouncedSessions ----- \n");
    // We are only interested in 200 OK requests, ofcourse
    if(*theStatusCode != qtssSuccessOK)
        return QTSS_NoErr;
      qtss_printf("-- TEst 2 Relay SetupAnnouncedSessions ----- \n");
    ...
    if (theElem == NULL)
    {
     ...
     qtss_printf("-- TEst 3 Relay SetupAnnouncedSessions ----- \n");
    // If theElem is not NULL
    if(setup)                       // No need to add it to the queue twice
        return QTSS_NoErr; 
    qtss_printf("-- TEst 4 Relay SetupAnnouncedSessions ----- \n");
    ...
    StrPtrLen theURL(theURLStr, theLen);
     qtss_printf("-- 222 Relay SetupAnnouncedSessions theURLStr=%s----- \n", theURLStr);    
    ...
    qtss_printf("-- 333 Relay SetupAnnouncedSessions thePort=%s----- \n", thePort);    
