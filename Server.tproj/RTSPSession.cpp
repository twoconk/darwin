...
{
    qtss_printf("### RTSPSession::~RTSPSession()\n");
...
SInt64 RTSPSession::Run()
{
    ...
    {
        qtss_printf("### RTSPSession::Run Task::kTimeoutEvent \n");
        fLiveSession = false;
    }
    ...
    while (this->IsLiveSession())
    {
        ...
        switch (fState)
        {
            ...
            {
                // qtss_printf("### RTSPSession::Run kReadingRequest \n");
            ...
            case kFilteringRequest:
            {
                ...
                {
                    qtss_printf("### RTSPSession::Run kFilteringRequest, IsDataPacket \n");
            ...
            case kAuthenticatingRequest:
            {
                ...
                // qtss_printf("### RTSPSession::Run kAuthenticatingRequest \n");
...
void RTSPSession::SetupRequest()
{
   ...
   // let's also refresh RTP session timeout so that it's kept alive in sync with the RTSP session.
    ...
    if (fRTPSession == NULL)
    {
        qtss_printf("RTSPSession::SetupRequest, CreateNewRTPSession\n");
