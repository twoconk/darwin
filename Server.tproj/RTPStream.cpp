...
//lyz@xdja.com add for update nat port.
pthread_t gid_thread = -1;
//add end
    ...
    fMonitorAudioDestPort(QTSServerInterface::GetServer()->GetPrefs()->GetUDPMonitorAudioPort() )
{
    ...
    //lyz@xdja.com add for nat port
    bNeedWaitRTCPPort = true;
    bNeedWaitRTPPort = true;
    bQuit = false;
    bRunning = false;
	//add end.
...
RTPStream::~RTPStream()
{
    ...
    //lyz@xdja.com add for update nat port
    bRunning = false;
    if (gid_thread != -1 && bRunning){
        qtss_printf("need wait nat thead");
        bQuit = true;
        //pthread_join(gid_thread,NULL);
    }
    //add end.
...
QTSS_Error RTPStream::Setup(RTSPRequestInterface* request, QTSS_AddStreamFlags inFlags)
{
    ...
    if (request->GetDestAddr() != INADDR_ANY)
    {
        ...
        if (!(inFlags & qtssASFlagsAllowDestination)){
            ...
            qtss_printf("Setup request->GetDestAddr():%d\n", request->GetDestAddr());
    ...
    qtss_printf("Setup request->fRemoteRTPPort:%d, fRemoteRTCPPort:%d, pool:%08x, this:%08x\n", fRemoteRTPPort, fRemoteRTCPPort, QTSServerInterface::GetServer()->GetSocketPool(), this);
    ...
    //lyz@xdja.com add for need care.
    if ((fRemoteRTPPort & 1) != 0){
        
            qtss_printf("Setup request->fRemoteRTPPort():%d\n", fRemoteRTPPort);
 ...
 //       return QTSSModuleUtils::SendErrorResponse(request, qtssClientBadRequest, qtssMsgRTCPPortMustBeOneBigger);       
    ...
    if (SocketUtils::IsMulticastIPAddr(fRemoteAddr))
    {
        ...
        if (fSockets != NULL)
        {
            ...
            if (err != QTSS_NoErr){
    				...
    				qtss_printf("--------- ReflectorStream::BindSockets, QTSS_NoErr != err:%d", err);
    ...
    //start wait nat packege by lyz@xdja.com
...
QTSS_Error  RTPStream::Write(void* inBuffer, UInt32 inLen, UInt32* outLenWritten, UInt32 inFlags)
{
    ...
    // qtss_printf("RTPStream::Write inLen=%d, inFlags=%08x\n", inLen, inFlags);
    //
    ...
    if (inFlags & qtssWriteFlagsIsRTCP) // 00000005 & 0x00000002 (101 & 010 = 0)
    {   
        // qtss_printf("RTPStream::Write, qtssWriteFlagsIsRTCP inLen=%d, inFlags=%08x\n", inLen, inFlags);
		//
    ...
    else if (inFlags & qtssWriteFlagsIsRTP) // 00000005 & 0x00000001 (101 & 001 = 1)
    {
        ...
        if (this->UpdateQualityLevel(thePacket->packetTransmitTime, theCurrentPacketDelay, theTime, inLen))
        {
			...
			{            
			if (bRunning){
                        qtss_printf("wait 200ms for nat !!\n");
                        OSThread::Sleep(200);
                        //sleep(100);
                    } 
                        //qtss_printf("write fRemoteRTPPort:%d\n", fRemoteRTPPort);
        ...
        if ( err == QTSS_NoErr && inLen > 0 )
        {
                ...
                (theTime > (fLastSenderReportTime + (senderReportInterval * 1000))))
            {
                ...
                // qtss_printf("RTPStream::Write SendRTCPSR 1370 line \n");
            }
...
void* thread_for_nat(void *parms){
    ...
    while(1){
        ...
        char szBuff[128];
        UInt32 iBufLen = sizeof(szBuff) - 1;
        UInt32 iRecvLen = 0;
