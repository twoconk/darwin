...
RTPSession::~RTPSession()
{
    qtss_printf("### RTPSession::~RTPSession() \n");
...
{
    qtss_printf("### RTPSession::Teardown \n");
...
SInt64 RTPSession::Run()
{
        ...
        if (!fModuleDoingAsyncStuff)
    {
        ...
        if (!sessionTable->TryUnRegister(&fRTPMapElem))
        {
            ...
            {
                qtss_printf("### RTPSession::Run    582 line ,fClosingReason=%d \n", fClosingReason );
        ...
        UInt32 numModules = QTSServerInterface::GetNumModulesInRole(QTSSModule::kClientSessionClosingRole);
        {
            ...
            {  
			    ...
			    (void)theModule->GetValueAsString (qtssModName, 0, &moduleName);

                qtss_printf("### RTPSession::Run    605 line ,QTSS_ClientSessionClosing_Role moduleName:%s\n", moduleName);
                 delete moduleName;
