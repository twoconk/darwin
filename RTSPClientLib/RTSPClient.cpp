...
{
    if (fVerboseLevel >= 4)
    qtss_printf("############### RTSPClient::SendDescribe  start, fState=%d \n", fState);
    if (!IsTransactionInProgress())
    {
        ...
        qtss_printf("RTSPClient::SendDescribe, fmt = %s\n", fmt.GetAsCString());
		fmt.PutTerminator();
    }
     if (fVerboseLevel >= 4)
    qtss_printf("############## RTSPClient::SendDescribe  end\n");
...
OS_Error RTSPClient::SendTeardown()
{
    if (!IsTransactionInProgress())
    {   
        qtss_printf("RTSPClient::SendTeardown \n");
...
OS_Error RTSPClient::DoTransaction()
{
    ...
    if (fVerboseLevel >= 4)
    qtss_printf("############ RTSPClient::DoTransaction come in fState=%d \n\n", fState);
	...
	for(;;)
	{
		switch(fState)
		{
			...
			case kInitial:
        		...
        		fPacketDataInHeaderBufferLen = 0;
                if (fVerboseLevel >= 3)
				qtss_printf("############ RTSPClient::DoTransaction kInitial\n");
			...
			case kRequestSending:
				 if (fVerboseLevel >= 4)
				qtss_printf("-----REQUEST-----len=%"_U32BITARG_"\n%s\n", theRequest.Len, STRTOCHAR(&theRequest));
			...
			case kHeaderReceived:
			 if (fVerboseLevel >= 4)
				qtss_printf("############ RTSPClient::DoTransaction kResponseReceiving \n\n\n\n");
    ...
    if (fVerboseLevel >= 4)
	 qtss_printf("############ RTSPClient::DoTransaction end  fState=%d\n", fState);
