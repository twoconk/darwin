...
//lyz@xdja.com add for libuv start.
#include <uv.h>
//#include <pthread.h>
...
struct UVWorkData{
    ...
    UInt32 inLength;
    UInt32 inFlag;
    ReflectorOutput* outPut;
    UInt64 id ;
    OSMutex *fOutputMutex;

};
//add end.

typedef struct LibUVClient{
    int aid;
    int vid;

    uv_loop_t* mWorkerVideoLoop;
    int refVideoLoop;
    uv_loop_t* mWorkerAudioLoop;
    int refAudioLoop;
    uv_thread_t mWorkerAudioThread;
    uv_thread_t mWorkerVideoThread;
}LibUVClient;
...
class ClientSession : public Task
{
    public:
        ...
        ClientSession(  UInt32 inAddr, UInt16 inPort, char* inURL,
                        ...
                        char *controlID = NULL, char *name = NULL, char *password = NULL, char* streamName = NULL);
        ...
        OSRef*          GetRef()                    { return &fRef; }  // zfl add
        ...
        void AddOutput(ReflectorOutput* inOutput, Bool16 isVideo);
        void RemoveOutput(ReflectorOutput* inOutput, Bool16 isVideo);
	   ...
	   //Returns kUInt32_Max if there is no track with such trackID
        ...
        void SetSDPContext(StrPtrLen& sdp) { 
            if (pSDPStr != NULL){
                free(pSDPStr);
            }
            pSDPStr = (char *)malloc(sdp.Len+1);
            if (pSDPStr){
                memcpy(pSDPStr, sdp.Ptr, sdp.Len);
                *(pSDPStr + sdp.Len) = '\0';
            }
        }
        char* GetSDPContext() {return pSDPStr;}
    ...
    public:
        ...
        // StrPtrLen   strName;
        char *pSDPStr;
        SVector<ReflectorOutput*>     fvoutput; // just video outputVector
        SVector<ReflectorOutput*>     faoutput; // just audio outputVector
        pthread_t fid_vthread;
        pthread_t fid_athread;
        pthread_t fid_rthread;
        OSMutex   fthreadMutex;
        //---------- zfl add end
        ...
        //UInt32          fOptionsIntervalInSec;   //;lyz@xdja.com del
        
        //Bool16          fOptions;//;lyz@xdja.com del
        ...
        //UInt32 fGuarenteedBitRate;//;lyz@xdja.com del
        //UInt32 fMaxBitRate;//;lyz@xdja.com del
        //UInt32 fMaxTransferDelay;//;lyz@xdja.com del
		Bool16 fEnableForcePlayoutDelay;
		UInt32 fPlayoutDelay;
        //UInt32 fBandwidth;				//bps//;lyz@xdja.com del
    ...
    public:
        void startConnect(ReflectorOutput* vOutPut, ReflectorOutput* aOutPut);
        void stopConnect(ReflectorOutput* vOutPut, ReflectorOutput* aOutPut);
        void start_thread_for_getmedia();
    ...
    private:    
        ...
        uv_thread_t mWorkerVideoThread;
        #endif 
        LibUVClient *pLibUVClient;
        //fvouput mutext        
        OSMutex     fOutputMutex;
     
        void initLibUVClient();
        void delLibUVClient();
        ...
        void putRtpToVideoQueue(char* inPacket, UInt32 inLength, UInt32 inFlag);
        void putRtpToAudioQueue(char* inPacket, UInt32 inLength, UInt32 inFlag);
};
